...
#
# Modified By M.Taylor, University of Reading:
# to read filtered AVHRR noise parameters
 
if [ $# -ne 3 ]
then
    echo "USAGE: ./convert_marine_filtered.sh instr(two character) year flag(y/n)"
    exit
fi
...
mkdir -p $instr/$yr
cd $instr/$yr
ln -s ../../convert_marine_filtered.py .
...
do
    ...
    # --------------------------------------------------------------------------------------------------
    if [ $flag -eq 'n' ]; then
	echo "python convert_marine_filtered.py --day "$day "--complevel 9 "$instr" "$year > $script
    else
	echo "python convert_marine_filtered.py --filtered --day "$day "--complevel 9 "$instr" "$year > $script
    fi
    # --------------------------------------------------------------------------------------------------
